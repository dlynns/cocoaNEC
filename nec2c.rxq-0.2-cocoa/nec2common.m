...
static void conect( int ignd )
{
  ...
  if ( n != 0 )
  {
	...
	for ( j = 1; j < n ; j++)
	{
	  ...
	  sep = fabsl( xi1- x[ic])+ fabs(yi1- y[ic])+ fabs(zi1- z[ic]) ;
	  if ( sep <= slen)
	  {
	    icon1[i]= -(ic+1 ) ;
	    break;
	  ...
	  sep = fabsl( xi1- x2[ic])+ fabs(yi1- y2[ic])+ fabs(zi1- z2[ic]) ;
	  if ( sep <= slen)
	  {
	    icon1[i]= (ic+1 ) ;
	    break;
	...
	sep = fabs(xi2- x[ic])+ fabsl(yi2- y[ic])+ fabs(zi2- z[ic]) ;
	if ( sep <= slen)
	{
	  icon2[i]= (ic+1 ) ;
	  break;
	...
	sep = fabsl(xi2- x2[ic])+ fabs(yi2- y2[ic])+ fabs(zi2- z2[ic]) ;
	if ( sep <= slen)
	{
	  icon2[i]= -(ic+1 ) ;
	  break;
	...
	for ( iseg = 0 ; iseg < n ; iseg++ )
	{
	  ...
	  slen =( fabsl(xi2- xi1 )+ fabs(yi2- yi1 )+ fabs(zi2- zi1 ))* SMIN ;
	  sep = fabsl(xi1- xs)+ fabs(yi1- ys)+ fabs(zi1- zs) ;
	  ...
	  sep = fabs(xi2- xs)+ fabs(yi2- ys)+ fabsl(zi2- zs) ;
	  if ( sep <= slen)
	  {
	    icon2[iseg]=PCHCON+ i;
	    ic = 0 ;
    ...
    doubletype a2, doubletype b2, doubletype rad, int ns, int itg )
{
  ...
  if ( a2 != a1 )
  {
    sangle= atan( a2/( fabsl( hl )+( fabs( hl )* a1 )/( a2- a1 ))) ;
    ...
    doubletype *zlr, doubletype *zli, doubletype *zlc )
{
  ...
  while( TRUE )
  {
	...
	case 6:
      ...
      if (( fabsl( crealx( zarray[i]))+ fabs( cimagx( zarray[i]))) > 1.0e-20 )
	iwarn =TRUE;
      zarray[i] += zt;
      ...
      for ( kth = 1; kth <= nth ; kth++ ) {
			...
			if ( ifar != 1 ) {
				if ( (ethm2 <= 1.0e-20 ) && (ephm2 <= 1.0e-20 ) ) {
					...
					if ( fabsl(dfaz) > fabs(dfaz2) ) dfaz = dfaz2;
...
static void reflc( int ix, int iy, int iz, int itx, int nop )
{
  ...
  if ( ix >= 0 )
  {
	...
	for ( i = 0 ; i < n ; i++ )
	{
	  ...
	  if ( (fabsl(e1 )+fabs(e2) <= 1.0e-5) || (e1*e2 < -1.0e-6) )
	  {
	    fprintf( output_fp,
