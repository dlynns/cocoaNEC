	...
	if ( ( token & LEXPREFIX ) == STATEMENT ) {
		...
		case RETURNSTATEMENT:
			...
			if ( token == SEMICOLON ) {
				...
				if ( functionType != VOIDTYPE ) [ parser setError:[ NSString stringWithFormat:@"return statement of a non-void function requires a value. %s", [ parser tokenType:token ] ] flush:YES ] ;
			...
			if ( functionType == VOIDTYPE ) {
				[ parser setError:[ NSString stringWithFormat:@"returning a value in a void function. %s", [ parser tokenType:token ] ] flush:YES ] ;
			}
			else {
				...
				if ( expressionType != functionType ) {
					[ parser setError:[ NSString stringWithFormat:@"returning a value that is incompatible with function type. %s", [ parser tokenType:token ] ] flush:NO ] ;
				}
			}
			token = [ parser token ] ;
			if ( token != SEMICOLON ) {
